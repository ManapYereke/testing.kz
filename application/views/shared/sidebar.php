<div class="sidebar">
    <ul class="navbar-nav">
        <a class="dropdown-item" href="<?= site_url("user/lst") ?>">Пользователи</a>
        <a class="dropdown-item" href="<?= site_url("edutype/lst") ?>">Виды обучения</a>
        <a class="dropdown-item" href="<?= site_url("speciality/lst") ?>">Специальности</a>
        <a class="dropdown-item" href="<?= site_url("position/lst") ?>">Должности</a>
        <div class="dropdown-divider"></div>
        <a class="dropdown-item" href="<?= site_url("subtest/lst") ?>">Субтесты</a>
        <a class="dropdown-item" href="<?= site_url("question/lst") ?>">Вопросы</a>
        <div class="dropdown-divider"></div>
        <a class="dropdown-item" href="<?= site_url("test/dlJSON") ?>">Выгрузить данные JSON</a>
        <a class="dropdown-item" href="<?= site_url("test/dlDefault") ?>">Выгрузить данные Default</a>
    </ul>
</div>