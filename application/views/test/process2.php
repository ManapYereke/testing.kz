...
<div class="container">
    <form method="post" action="<?= site_url($this->uri->segment(1) . "/save") ?>">
		...
		foreach ($tb0301 as $r1) {
			...
			$step++;
		?>
        <div id="tb0301-<?= $r1->tb0301_id ?>" class="card mt-2 step step<?= $step ?> <?= $step == 1 ? "" : "d-none" ?>"
            step="<?= $step ?>" tb0301_timelimit="<?= $r1->tb0301_timelimit ?>">
            <h5 class="card-header"><?= $this->utils->t([$r1->tb0301_name_ru, $r1->tb0301_name_kz]) ?> <span
                    class="badge badge-secondary">00:00</span></h5>
            <div class="card-body">
                <div class="intro">
                ...
                <div class="questions d-none">
                        ...
                        <div class="nav nav-pills" id="nav-tab" role="tablist">
                            <?
								...
								foreach ($tb0302 as $r2) {
									...
									$question++;
								?>
                            <a class="nav-item nav-link <?= $step ?><?= $question ?> <?= $question == 1 ? "active" : "" ?>"
                                id="tab-<?= $r2->tb0302_id ?>-title" name="question link" data-toggle="tab"
                                href="#tab-<?= $r2->tb0302_id ?>" role="tab" aria-selected="true"><?= $i ?></a>
                            <? } ?>
                        </div>
                    </nav>
                    <div class="tab-content pt-2" id="nav-tabContent">
                        <? $question = 0;
							foreach ($tb0302 as $r2) {
								$question++; ?>
                        <div class="tab-pane fadeshow <?= $step ?><?= $question ?> <?= $tb0302[0] == $r2 ? "active" : "" ?>"
                            id="tab-<?= $r2->tb0302_id ?>" role="tabpanel">
                            ...
                            <table class="table table-striped table-hover">
                                <?
										...
										?>
                                <tr>
                                    <th width="1%"><input type="radio" name="tb0302_id-<?= $r2->tb0302_id ?>"
                                            onclick="atLeastOneRadio(this.name, <?= $step?>, <?= $question?>)"
                                            value="<?= $x ?>"></th>
                                    <td>
                                        <?= $r2->$f ?>
                                    </td>
                            ...
                            <div class="card-footer text-right d-none">
                                <button id="btGoToNextQuestion" onclick="nextQuestion(<?= $step?>, <?= $question?>)"
                                    type="button" class="btn btn-secondary <?= !$lastquestion?"":"d-none"?>"
                                    style="background-color: #007bff">Следующий вопрос</button>
                                <button id="btGoToNextSubTest" type="button" onclick="stepEnd(<?= $step + 1 ?>)"
                                    class="btn btn-secondary <?= $step==3 || !$lastquestion ? "d-none" : "" ?>"
                                    style="background-color: #007bff">ЗАВЕРШИТЬ</button>
                                <button id="btGoToFinish" type="button" onclick="finish()"
                                    class="btn btn-warning <?= $step==3 && $lastquestion ?"" : "d-none" ?>">Завершить
                                    тест</button>
                            </div>
                        </div>
                        <? } ?>
